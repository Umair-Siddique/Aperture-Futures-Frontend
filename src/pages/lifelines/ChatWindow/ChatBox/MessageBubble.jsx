...
const MessageBubble = ({ msg, isUser, isFirstAIResponse }) => {
  ...
  const getMessageStyles = (from, isError) => {
    if (from === "user") {
      // Light green background for sent messages, no border
      return "bg-green-100 text-green-900 shadow-green-200/25";
    }
    if (isError) {
      return "bg-gradient-to-br from-red-900/20 to-red-800/20 text-red-200 border border-red-700/50";
    }
    // AI message style
  return "bg-[#FCFCFC] text-black shadow-black/10";
  };

  return (
    <div className="relative group/message">
      <div
        className={`relative px-4 py-3 md:px-6 md:py-4 shadow-sm text-sm md:text-base leading-relaxed transition-all duration-200 hover:shadow-md
          ${getMessageStyles(msg.from, msg.isError)}
          ${isUser ? "rounded-br-md" : "rounded-bl-md"}`}
        style={{ borderRadius: '11.54px' }}
      >
        <p
